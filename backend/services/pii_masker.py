...
- SSNs (123-45-6789) — unambiguous format.
...
def _mask_card_candidate(match: re.Match) -> str:
    ...
    if 13 <= len(digits) <= 19 and _luhn_valid(digits):
        return CARD_TOKEN
    return match.group()  # not a card (e.g. a long numeric ID) — leave it


def mask_pii(text: str | None) -> str | None:
    """Replace SSNs and valid card numbers with redaction tokens."""
    if not text:
        return text
