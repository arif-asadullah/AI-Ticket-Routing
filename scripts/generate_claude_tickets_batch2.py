...
tickets = [
    ...
    # Access Management (4)
    t("cant attach files to email — too large", "trying to email a 30mb presentation to a client but it keeps bouncing back saying too large. this is a normal business file why is there such a small limit?? tried compressing it still too big", "Access Management", "medium", ["prod-mail-01"], ["exchange"], [], None, "frustrated_user"),
    t("mapped network drive disappeared", "my Z: drive that maps to the shared department folder just disappeared from my computer. tried mapping it again but 'network path not found'. all my project files are there!!!", "Access Management", "high", ["prod-nfs-01"], ["nfs"], ["ERR-NFS-001"], "KB-0006", "frustrated_user"),
    t("recycle bin empty but disk still full", "my computer says disk full but i emptied the recycle bin and deleted everything i could. still says 2% free. where is all the space going?? cant even save documents", "Access Management", "medium", [], [], ["ERR-SYS-002"], "KB-0008", "frustrated_user"),
    t("photos from company event gone", "the marketing team photos from last weeks company event that were on the shared drive are all gone. someone deleted them or the folder was moved. we need these for the newsletter going out friday", "Access Management", "high", ["prod-nfs-01"], ["nfs"], [], None, "frustrated_user"),
    ...
    # Access Management (5)
    t("Cloud storage migration ROI analysis requested", "CFO wants ROI analysis for migrating on-premise NFS storage to AWS S3/EFS. Current on-prem storage: 50TB, growing 500GB/month. Need cost comparison, performance implications, and migration approach.", "Access Management", "medium", ["prod-nfs-01"], ["nfs"], [], None, "manager"),
    t("Data classification project needed for storage tiering", "Storage costs are growing 15% quarterly. CTO wants hot/warm/cold data classification to move infrequently accessed data to cheaper storage. Need assessment of data access patterns and tiering strategy.", "Access Management", "medium", ["prod-nfs-01", "prod-db-01"], ["nfs", "postgresql"], [], None, "manager"),
    t("Ransomware protection for backup infrastructure", "After seeing recent ransomware attacks in the industry, the CISO wants immutable backups implemented. Current backups on prod-nfs-01 can be modified or deleted. Need air-gapped or immutable backup solution.", "Access Management", "high", ["prod-nfs-01"], ["nfs"], [], None, "manager", secondary="Security"),
    t("Storage performance metrics needed for SLA reporting", "Operations team needs monthly storage performance metrics (IOPS, latency, throughput) for client SLA reporting. Currently collected manually. Need automated reporting solution integrated with Grafana dashboards.", "Access Management", "low", ["prod-nfs-01"], ["nfs", "grafana"], [], None, "manager"),
    t("End-of-life NAS appliance replacement needed", "The NAS appliance supporting prod-nfs-01 reaches end-of-life in August. Vendor will stop providing firmware updates. Need replacement evaluation: buy new appliance vs migrate to software-defined storage vs cloud.", "Access Management", "high", ["prod-nfs-01"], ["nfs"], [], None, "manager"),
]
